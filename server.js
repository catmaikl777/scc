...
wss.on("connection", async (ws, req) => {
  ...
  try {
    ...
    console.log(`✅ New connection established with session ${sessionId}, waiting for name...`);
  } catch (error) {
      ...
      switch (message.type) {
        ...
        case "leave_room":
          if (message.roomId && rooms.has(message.roomId)) {
            ...
            if (room.users.size === 0) {
              console.log(`🗑️ Room ${message.roomId} is empty, scheduling cleanup`);
              room.isActive = false;
              setTimeout(() => {
                if (rooms.has(message.roomId) && rooms.get(message.roomId).users.size === 0) {
        ...
        case "end_call":
          if (message.roomId && rooms.has(message.roomId)) {
            ...
            console.log(`📞 Call ended by ${userName} in room ${message.roomId}`);
  ...
  ws.on("close", async (code, reason) => {
    ...
    rooms.forEach((room, roomId) => {
      if (room.users.has(sessionId)) {
        ...
        if (room.users.size === 0) {
          console.log(`🗑️ Room ${roomId} is empty, removing`);
          room.isActive = false;
          setTimeout(() => {
