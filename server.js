...
wss.on("connection", async (ws, req) => {
  ...
  try {
    // УДАЛИТЕ создание временного имени - вместо этого будем ждать setName от клиента
    // Просто создаем сессию без пользователя
    await db.createUserSession(null, sessionId); // Передаем null для userId

    // Сохраняем в памяти с пустым пользователем
      ...
      switch (message.type) {
        case "setName":
          if (message.name && message.name.trim()) {
            ...
            try {
              // ИСПРАВЛЕНИЕ: Сначала создаем/находим пользователя
              ...
              // Обновляем сессию с правильным userId
              await db.createUserSession(userId, sessionId);

              // Проверяем, было ли у пользователя предыдущее имя (для случая смены имени)
  ...
  ws.on("close", async (code, reason) => {
    ...
    try {
      ...
      // ИСПРАВЛЕНИЕ: Проверяем, что пользователь был установлен
      if (clientData && currentUser && userId) {
        ...
        // Если пользователь не установил имя, просто удаляем сессию
        await db.endUserSession(sessionId);
        clients.delete(sessionId);
        console.log(`✅ Anonymous session ${sessionId} removed`);
      }
    } catch (error) {
