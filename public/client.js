  ...
  async function initializeLocalStream() {
    try {
      ...
      if (audioTracks.length === 0) {
        console.warn("⚠️ No audio tracks in stream, requesting audio only...");
        ...
        // Объединяем потоки если есть видео
      ...
      // Пробуем получить только аудио как запасной вариант
      try {
        localStream = await navigator.mediaDevices.getUserMedia({
          audio: {
            echoCancellation: true,
      ...
      pc.ontrack = (event) => {
        ...
        if (event.streams && event.streams[0]) {
          ...
          console.log("🔊 Remote stream tracks:", {
            ...
            audioTrack: remoteStream.getAudioTracks()[0]?.kind,
            videoTrack: remoteStream.getVideoTracks()[0]?.kind
          });
