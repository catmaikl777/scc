(() => {
  ...
  async function startVoiceRecording() {
    try {
      ...
      // Сначала проверяем поддержку MediaRecorder
      if (typeof MediaRecorder === "undefined") {
        showSystemMessage("❌ Ваш браузер не поддерживает запись аудио");
        return;
      }

      // Запрашиваем доступ к микрофону с более простыми настройками
      ...
      if (!supportedType) {
        // Если не поддерживается ни один тип, используем базовый
        supportedType = "audio/webm";
      }

      console.log("🎵 Using audio format:", supportedType);

      // Создаем MediaRecorder с выбранным форматом
  ...
  function handleVoicePlayback(event) {
    ...
    if (audioData.startsWith("Ukl")) mimeType = "audio/wav";
    if (audioData.startsWith("Ogg")) mimeType = "audio/ogg";
  ...
  function handleInitMessage(message) {
    ...
    // ВАЖНО: Всегда используем сохраненное имя из localStorage
    ...
    if (savedName && nameInput) {
      ...
      // НЕМЕДЛЕННО отправляем имя на сервер
      setTimeout(() => {
        if (isConnected) {
      ...
      // Только если нет сохраненного имени - генерируем новое
      const autoName = `User${Math.floor(Math.random() * 10000)}`;
