(() => {
  ...
  async function createOffer(targetSessionId, attempt = 1) {
    ...
    // УЛУЧШЕННАЯ ПРОВЕРКА: разрешить создание оффера даже если есть существующее соединение
    ...
    if (existingPc) {
      if (existingPc.signalingState === "have-local-offer") {
        ...
        // Вместо возврата, ждем и проверяем снова
        if (attempt < 3) {
          setTimeout(() => createOffer(targetSessionId, attempt + 1), 2000);
        }
        return;
      }
      ...
      // Если соединение в плохом состоянии - пересоздаем
      if (existingPc.signalingState === "closed" || existingPc.connectionState === "failed") {
        console.log(`🔄 Replacing failed connection with ${targetSessionId}`);
        existingPc.close();
  ...
  async function handleRoomUsers(message) {
    ...
    // Создаем соединения только для тех, у кого их нет
    for (let i = 0; i < otherUsers.length; i++) {
      ...
      if (!existingPc ||
        ...
        console.log(`🔄 Setting up new connection with: ${user.userName}`);
        ...
        console.log(`✅ Already have active connection with ${user.userName} (${existingPc.connectionState})`);
      }
    }
  }
  ...
  async function handleWebRTCOffer(message) {
    try {
      ...
      // УБРАТЬ КОНФЛИКТ ОФФЕРОВ - разрешить одновременные офферы
      if (peerConnections.has(message.fromSessionId)) {
        const existingPc = peerConnections.get(message.fromSessionId);

        // Вместо закрытия, проверяем только критические состояния
        if (existingPc.signalingState === "closed" || existingPc.connectionState === "failed") {
        ...
        // В других случаях продолжаем обработку оффера
      }
