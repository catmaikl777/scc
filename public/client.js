(() => {
  ...
  async function createOffer(targetSessionId, attempt = 1) {
    ...
    if (existingPc &&
      ...
      console.log(`✅ Already connected/stable with ${targetSessionId}, skipping offer`);
      return;
    }
  ...
  async function handleRoomUsers(message) {
    ...
    // Создаем соединения с задержкой между каждым
    for (let i = 0; i < otherUsers.length; i++) {
      ...
      if (existingPc &&
        (existingPc.connectionState === "connected" ||
          existingPc.signalingState === "have-local-offer")) {
        console.log(`⏭️  Already processing connection with ${user.userName}, skipping`);
        continue;
      }

      if (!peerConnections.has(user.sessionId)) {
        if (isCallInitiator) {
          console.log(`📤 Initiator creating offer for: ${user.userName}`);
          ...
          console.log(`⏳ Non-initiator waiting for offer from: ${user.userName}`);
          // Не-инициаторы тоже могут создавать офферы через 10 секунд ожидания
          setTimeout(() => {
            if (!peerConnections.has(user.sessionId) && isInCall) {
              console.log(`🔄 Non-initiator creating offer for ${user.userName} after timeout`);
    ...
    setTimeout(updateVideoGridLayout, 500);
  }
